...
const LiveRideRequest = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axiosBaseInstance.get(endpoints.GET_LIVE_RIDE_REQUESTS({ page, limit }));
      if (response.success) {
        setRideData(response.data.rides || response.data.rideRequests || []);
        setTotalCount(response.data.totalCount || 0);
        setTotalPages(response.data.pagination?.totalPages || 1);
      }
    } catch (err) { setError(err.message || 'Failed to fetch ride requests'); }
  ...
  if (error) return <p className="text-center text-red-600 py-3">{error}</p>;
