...
const styles = {
  ...
  filterBar: { display: "flex", alignItems: "center", gap: 32, marginBottom: 20 },
  ...
  formGrid: { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "16px 24px" },
  formGroup: { display: "flex", flexDirection: "column", gap: 6 },
  formLabel: { fontSize: 13, color: "#333", fontWeight: 500 },
  required: { color: "red" },
  formInput: { border: "1px solid #ccc", borderRadius: 6, padding: "8px 10px", fontSize: 14, outline: "none", background: "#fff" },
  saveBtn: { background: "#111", color: "#fff", border: "none", borderRadius: 20, padding: "8px 24px", fontSize: 14, fontWeight: 600, cursor: "pointer", display: "block", marginLeft: "auto", marginTop: 16 },
...
const historyData = Array(6).fill({ createdOn: "16 Jul 2025", couponCode: "QPO50", status: "Active" });
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.topBar}>
        <h2 className="poppins-title" style={styles.pageTitle}>Coupons and Offers</h2>
      ...
      <div style={styles.formSection}>
        ...
        <div style={styles.formGrid}>
          {["Coupon Title", "Coupon Code", "Discount Type", "Discount Value", "Max Discount", "Discount Value", "Per Day Limit", "Use Limit", "Minimum Amount to Apply", "Select Date Range", "Description"].map((label, i) => (
            <div key={i} style={styles.formGroup}>
              <label style={styles.formLabel}>{label}<span style={styles.required}>*</span></label>
              <input style={styles.formInput} value={form[label] || ""} onChange={(e) => setForm({ ...form, [label]: e.target.value })} />
            </div>
          ))}
        </div>
              ...
              <tr key={i} style={{ background: "#fff" }}>
                ...
                <td style={styles.td}>
                  <div style={styles.actionCell}>
                    <button style={styles.iconBtn}><EyeIcon /></button>
