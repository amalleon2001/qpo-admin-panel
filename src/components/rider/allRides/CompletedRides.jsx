...
function CompletedRides() {
  ...
  const fetchCompletedRides = useCallback(async () => {
    ...
    try {
      ...
      const response = await axiosTripInstance.get(endpoints.GET_COMPLETED_RIDES({ page, limit: 10, startDate: startDate.toISOString(), endDate: endDate.toISOString() }));
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={8} className="text-sm text-gray-500 py-8 text-center">
                  {fromDate === toDate
                    ? `No completed rides on ${new Date(fromDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}`
                    : `No completed rides from ${new Date(fromDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })} - ${new Date(toDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}`
                  }
                </td>
              </tr>
            ) : (
